...
describe("read tools", () => {
  it("returns an error object when access is denied (no data leak)", async () => {
    vi.mocked(q.checkBrandAccess).mockResolvedValue({
      ok: false,
      status: 404,
      error: "Brand not found",
    });
    const tools = buildReadTools(ctx);
    const out = await tools.get_brand_profile.execute?.(
      {},
  ...
  it("returns brand data when access is granted", async () => {
    ...
    vi.mocked(q.getBrandById).mockResolvedValue({
      id: "b1",
      name: "Acme",
      tone: "bold",
    } as never);
  ...
  it("blocks reading a conversation that belongs to a different brand (IDOR)", async () => {
    ...
    vi.mocked(q.getConversationById).mockResolvedValue({
      id: "c1",
      brandId: "b2",
    } as never);
    const tools = buildReadTools(ctx);
  ...
  it("returns messages when the conversation belongs to the active brand", async () => {
    ...
    vi.mocked(q.getConversationMessages).mockResolvedValue([
      { id: "m1", content: "hi" },
    ] as never);
    const tools = buildReadTools(ctx);
    const out = await tools.read_conversation.execute?.(
